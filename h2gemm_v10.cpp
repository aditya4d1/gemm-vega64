...
constexpr uint32_t xDim = 4096*2;
constexpr uint32_t yDim = 4096*2;
constexpr uint32_t xDim4 = xDim/8;
constexpr uint32_t xDim8 = xDim/16;
constexpr uint32_t x32 = 32;
constexpr uint32_t x16 = 16;
constexpr size_t Size = xDim * yDim * sizeof(short);

__global__ void SGEMM(Half2x4 *A, Half2x4 *B, Half2x4 *C, int *getGlobalAId, int *getGlobalCId) {
    ...
    for(int j=1;j<yDim/8;j++) {
        ...
        // i = 0
        outerProduct8x8(rA[0], rB[0], c[0], c[1], c[2], c[3]);
        outerProduct8x8(rA[0], rB[1], c[4], c[5], c[6], c[7]);
        outerProduct8x8(rA[1], rB[0], c[8], c[9], c[10], c[11]);
        outerProduct8x8(rA[1], rB[1], c[12], c[13], c[14], c[15]);

        shared_read_b128<2*512>(rA[0], rA[1], rB[0], rB[1], ldsReadA, ldsReadB);
        lgkmcnt<4>();

        // i = 1
        outerProduct8x8(rA[2], rB[2], c[0], c[1], c[2], c[3]);
        outerProduct8x8(rA[2], rB[3], c[4], c[5], c[6], c[7]);
        outerProduct8x8(rA[3], rB[2], c[8], c[9], c[10], c[11]);
        outerProduct8x8(rA[3], rB[3], c[12], c[13], c[14], c[15]);
...
int main() {
    hipSetDevice(1);
    std::vector<short> a(xDim*yDim), b(xDim*yDim), c(xDim*xDim);
    std::fill(c.begin(), c.end(), 0);
    short *_a = reinterpret_cast<short*>(a.data());
    short *_b = reinterpret_cast<short*>(b.data());
    short *_c = reinterpret_cast<short*>(c.data());

    for(int j=0;j<yDim;j++) {
        for(int i=0;i<xDim;i++) {
            _a[i + j * xDim] = (j+i*xDim)*1 + 1;
            if(i == j) {
            _b[i + j * xDim] = 1;
            } else {
            _b[i + j * xDim] = 0;
    ...
    hipMalloc(&Ad, a.size()*sizeof(short));
    hipMalloc(&Bd, b.size()*sizeof(short));
    hipMalloc(&Cd, c.size()*sizeof(short));
    hipMemcpy(Ad, a.data(), a.size()*sizeof(short), hipMemcpyHostToDevice);
    hipMemcpy(Bd, b.data(), b.size()*sizeof(short), hipMemcpyHostToDevice);
    hipMemcpy(Cd, c.data(), c.size()*sizeof(short), hipMemcpyHostToDevice);
    ...
    hipMemcpy(c.data(), Cd, c.size()*sizeof(short), hipMemcpyDeviceToHost);
